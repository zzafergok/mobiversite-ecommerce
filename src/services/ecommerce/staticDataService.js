// Static data service for production environment (Vercel)
// This service provides the same interface as jsonServerService but uses static data
...
export const staticDataService = {
  ...
  async getUserByCredentials(username, password) {
    await delay()
    return usersData.find((u) => u.username === username && u.password === password) || null
  },
  ...
  async checkUsernameExists(username) {
    await delay()
    return usersData.some((u) => u.username === username)
  },
  ...
  async getUserCart(userId) {
    await delay()
    const userCarts = cartsData.filter((c) => c.userId === userId)
    if (userCarts.length > 0) {
      return userCarts.sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0))[0]
    }
    return null
  },
